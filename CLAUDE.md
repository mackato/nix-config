...
macOS（aarch64-darwin）の **home 環境を nix-darwin + home-manager で宣言管理する flake** の**個人レイヤー**。会社標準（共通 cask・共通 CLI・gh・git 中立設定・system/nix 設定・nix-homebrew）は flake input の [airs/nix-config](https://github.com/airs/nix-config)（会社標準レイヤー）が提供し、本リポジトリは `airs.lib.mkDarwinConfig` に個人モジュールを渡して 1 つの構成に合成する。対象はユーザー単位の CLI・dotfiles・GUI アプリ。単一マシン構成で、`flake.nix` の `darwinConfigurations.default` がエントリ（ホスト名には依存しない）。**公開リポジトリ**であり、コミット内容・全 Git 履歴は誰でも閲覧できる前提で扱う。
...
- **input 更新を伴う適用**: alias `dru`（= flake update してから switch）。会社標準（airs）の更新取り込みもこれで行う。
- `drs`/`dru` は `~/.zshrc.local` の `NIX_CONFIG_FLAKE`（このリポジトリのクローン先の絶対パス）と `NIX_CONFIG_ATTR`（未設定なら `default`）を参照する（設定手順は README）。

### 必須の落とし穴（[[nix-config-ops]] と同内容）
...
| ファイル | 役割 |
| --- | --- |
| `flake.nix` | inputs は **airs/nix-config のみ**（nixpkgs / nix-darwin / home-manager / nix-homebrew は airs 経由で解決）。`airs.lib.mkDarwinConfig { username, extraModules, extraHomeModules }` で個人モジュールを合成。`formatter`・`checks`・`devShells` は airs の nixpkgs を再利用 |
...
| `darwin/configuration.nix` | 個人の GUI cask のみ（airs 側の cask と自動連結マージ） |
| `home/home.nix` | 個人の `home.packages`（CLI）・`programs.*`（git identity/gpg/starship/zsh）・`home.file`（静的 dotfiles） |
| `home/files/` | home.nix が参照する静的ファイル。`zsh/init.zsh`→`programs.zsh.initContent`、`zsh/profile.zsh`→`profileExtra`、`ghostty/`・`zed/`・`claude/` |

**ツールをどこで管理するかの原則（重要）**:
- 会社標準のツール・設定 → **airs/nix-config**（本リポジトリには書かない。重複宣言しない）。
- 個人の CLI / 開発 CLI → **home-manager**（`home.packages` か `programs.*`）。**Homebrew formula としては入れない**。
- 個人の GUI アプリ → **`homebrew.casks`**（`darwin/configuration.nix`）。
...
- **`homebrew.onActivation.cleanup = "uninstall"`**（airs 側で宣言）: 宣言（airs と本リポの `casks` の和集合）に無い cask/formula は switch 時に自動アンインストールされる。GUI アプリを追加・存続させたいときは必ずどちらかの `casks` に足す。逆に手動 `brew install` した formula は switch で消える。
- **airs 側のスカラ値は `lib.mkDefault` で宣言されている**。個人側で変えたい場合は素の代入で上書きできる（リスト型 `casks`/`home.packages` は自動連結マージ）。
- **`home/files/claude/CLAUDE.md` はこのリポの開発ガイドではない**。ユーザーのグローバル `~/.claude/CLAUDE.md` を配布するための中身なので、リポ運用ルールと混同して編集しない。
- **git 設定は `programs.git.settings`**（新スキーマ）を使う。identity・署名・alias は本リポ、中立設定（`init.defaultBranch`・`fetch.prune`）は airs 側。書き出し先は XDG の `~/.config/git/config`。
...
- **stateVersion は airs の `mkDarwinConfig` が管理**（既定値 system=6 / home="26.05"）。本リポには書かない。変えたいときだけ引数で渡す。
