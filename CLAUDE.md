...
macOS（aarch64-darwin）の **home 環境を nix-darwin + home-manager で宣言管理する flake**。対象はユーザー単位の CLI・dotfiles・GUI アプリ・system/nix 設定。単一マシン構成で、`flake.nix` の `darwinConfigurations.default` がエントリ（ホスト名には依存しない）。
