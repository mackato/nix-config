...
let
  ...
  # Claude Code の user スコープ MCP サーバ（いずれも npx 起動の stdio サーバ。node が前提）。
...
in
{
  ...
  # mcpServers へ上記サーバを冪等に注入し、宣言外の他サーバと Claude 側の他状態（projects 等）は
  ...
  home.activation.claudeMcpServers = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ -n "''${DRY_RUN_CMD:-}" ]; then
      echo "(dry-run) would merge context7/playwright/chrome-devtools into mcpServers of $claudeJson"
    else
